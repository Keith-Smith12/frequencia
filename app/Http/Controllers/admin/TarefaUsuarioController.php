...
class TarefaUsuarioController extends Controller
{
    ...
    {
        $data['tarefaUsuarios'] = TarefaUsuario::join('usuarios', 'tarefa_usuarios.it_id_usuario', '=', 'usuarios.id')
            ->join('tarefas', 'tarefa_usuarios.it_id_tarefa', '=', 'tarefas.id')
            ->select(
                'tarefa_usuarios.*',
                'usuarios.vc_nome as nome_usuario',
                'tarefas.vc_nome as nome_tarefa'
            )
            ->get();
    
        $data['usuarios'] = \App\Models\Usuario::all();
